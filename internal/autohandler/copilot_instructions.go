...
func buildManagedBlock(outputDir string) string {
	...
	b.WriteString("**BLOCKING REQUIREMENT**: Before exploring any source file, opening any directory,\n")
	b.WriteString("or answering any question about the codebase, you MUST invoke the `understand-repo`\n")
	b.WriteString("skill first. Do not read `")
	b.WriteString(entrypoint)
	b.WriteString("` or any other file directly — invoke the skill\n")
	b.WriteString("and follow its instructions. Invoking the skill IS the required entry point;\n")
	b.WriteString("skipping it and going straight to files is not allowed.\n\n")
	b.WriteString("Use the linked files in the `")
	b.WriteString(filepath.ToSlash(outputDir))
	b.WriteString("/` directory (surfaced by the skill) to understand the codebase structure.\n")
